...
mod geometry_datums;
mod text_ops;
pub(crate) use engine_fields::*;
pub(crate) use geometry_datums::*;
pub(crate) use text_ops::*;
...
pub(crate) fn assign_datum_field(
    ...
    let client_mob_assignment = if field.as_str() == "mob" && state.client.has_session(datum) {
        match &value {
            Value::Datum(mob) => {
                let path = state
                    .heap
