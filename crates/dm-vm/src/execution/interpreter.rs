//! The interpreter instruction dispatcher: the once-enormous `match` over bytecode
//! instructions, factored out of `run` so the dispatch loop stays navigable.
//!
...
pub(crate) enum DispatchFlow {
    /// Re-enter the loop without advancing the frame instruction pointer.
    Continue,
...
///
/// A later pass folds these dispatch parameters into an `Interpreter`
/// context struct; until then the parameter list stays explicit.
...
pub(crate) fn dispatch_instruction(
    ...
    match instruction {
        ...
        Instruction::PushFile(path) => frames[frame_index].stack.push(Value::file(path.as_str())),
        ...
        Instruction::AllocateCurrentDatum { argument_count } => {
            ...
            let root_len = preserve_reentrant_frame_roots(state, frames);
            let allocated = allocate_or_replace_engine_datum(state, type_path.clone(), &arguments);
            state.host_value_roots.truncate(root_len);
            let datum = allocated.map_err(|message| execution_error(module, frames, message))?;
        ...
        Instruction::MakeRegex { argument_count } => {
            ...
            let flags_name =
                FieldName::parse("flags").expect("the built-in regex flags field name is valid");
        ...
        Instruction::StandardBuiltin {
                ...
                .map_err(|message| execution_error(module, frames, message))?
            {
                ...
                return Ok(DispatchFlow::Exit(Box::new(Ok(
                    FrameRunOutcome::Prompted {
                        id: prompt.id,
                        prompt: PendingLocalPrompt {
                            ...
                            continuation: PendingPromptContinuation::Frames(std::mem::take(frames)),
                        },
                    },
                ))));
            }
        ...
        Instruction::Animate {
            ...
            for (index, (name, value)) in argument_names.iter().zip(arguments).enumerate() {
                if expanded_indices
                    .binary_search(&to_local_index(index).expect("animate argument count is u16"))
                    .is_ok()
                {
                    let Value::List(list) = value else {
                        return Err(execution_error(
        ...
        Instruction::MakeFilter {
            ...
            for (index, (name, value)) in argument_names.iter().zip(arguments).enumerate() {
                if expanded_indices
                    .binary_search(&to_local_index(index).expect("filter argument count is u16"))
                    .is_ok()
                {
                    let Value::List(list) = value else {
                        return Err(execution_error(
        ...
        Instruction::Sleep => {
            ...
            return Ok(DispatchFlow::Exit(Box::new(Ok(FrameRunOutcome::Yielded {
        ...
        Instruction::Block { argument_count } => {
            let count = usize::from(*argument_count);
            if !(count == 2 || (3..=6).contains(&count)) || frames[frame_index].stack.len() < count
            {
                return Err(execution_error(
                    module,
                    frames,
        ...
        Instruction::MakeListEntries(kinds) => {
            ...
            let Some(value_count) = value_count else {
                return Err(execution_error(module, frames, "list literal is too large"));
            };
            let stack_length = frames[frame_index].stack.len();
        ...
        Instruction::IndexList => {
                ...
                })
            {
                ...
                let key =
                    savefile_resolve_path(&state.savefiles.entry(savefile).or_default().cd, &key);
                let entry = state
                    .heap
                ...
                Value::Null
                    if frames.len() > 1
                        && module.procedure_path(procedure).is_some_and(|path| {
                            path == "/datum/proc/_SendSignal" || path.contains("/proc/_SendSignal@")
                        }) =>
                {
                    ...
                    let repaired = match (lookup, listener) {
                        (Some((sender, lookup)), Some(listener)) => {
                            let is_stale_scalar = state
                                ...
                                .is_some_and(|value| value.semantic_eq(&Value::Datum(listener)));
                            if is_stale_scalar {
                                let empty = {
                                    let lookup = state
                                        .heap
                                ...
                                if empty {
                                    state
                                        .heap
                                        .set_datum_field(sender, listen_lookup_field, Value::Null)
        ...
        Instruction::NextLocalListIteration {
            ...
            let Some(Value::List(list)) = frames[frame_index].locals.get(list_slot).cloned() else {
                return Err(execution_error(
                    module,
                    frames,
            ...
            let Some(Value::Number(index)) = frames[frame_index].locals.get(index_slot).cloned()
            else {
                return Err(execution_error(
                    module,
                    frames,
        ...
        Instruction::SetListIndex => {
            ...
            let args_write =
                (frames[frame_index].args_list == Some(list)).then(|| (key.clone(), value.clone()));
            if state.global_vars_proxy == Some(list) {
        ...
        Instruction::CompoundListIndex(operator) | Instruction::CompoundListIndexKeep(operator) => {
        ...
        Instruction::PrepareIteration => {
            ...
            if let (Value::List(snapshot), Some(assignment)) = (
                ...
                let value = match &assignment.value {
                    ...
                    SimpleIterationValue::TypePath(value) => Some(Value::TypePath(value.clone())),
                ...
                let datums = state.heap.list(*snapshot).ok().and_then(|values| {
                    values
                        ...
                        .map(|(_, value)| match value {
                            Value::Datum(datum) if state.heap.datum(*datum).is_ok() => Some(*datum),
                            _ => None,
                        })
            ...
            let value = match receiver {
                ...
                Value::Datum(datum) => {
                    ...
                    if let Some(value) = quickened_value {
                        ...
                        match value {
                            Ok(value) => value,
                            Err(ValueError::MissingField(_)) if statically_declared => Value::Null,
                            Err(error) => {
        ...
        Instruction::InitialDynamicField => {
            ...
            let value = if type_scope {
                ...
                initial_value_or_engine_root(state, &runtime_type, &field).unwrap_or(Value::Null)
            };
            frames[frame_index].stack.push(value);
        }
        ...
        Instruction::MutateLocal {
            ...
            let Some(current) = frames[frame_index].locals.get(usize::from(slot)).cloned() else {
                return Err(execution_error(
                    module,
                    frames,
                    format!("invalid local slot {slot}"),
        ...
        Instruction::LoadLocal(slot) => {
            let slot = *slot;
            let Some(mut value) = frames[frame_index].locals.get(usize::from(slot)).cloned() else {
                return Err(execution_error(
                    module,
                    frames,
                    format!("invalid local slot {slot}"),
                ...
                && let Some(mob) = state.client.attached_mob(*client)
            {
                ...
                if mob_is_pending {
                    state.attach_local_client(client, mob).map_err(|message| {
                        execution_error(module, frames, format!("client connection: {message}"))
                    })?;
                    let key = state
                        .heap
        ...
        Instruction::CallDynamic {
            ...
            if let Value::List(list) = receiver {
                ...
                let result = result.map_err(|message| execution_error(module, frames, message))?;
                frames[frame_index].stack.push(result);
                ...
                && is_regex_datum(*datum, state)
            {
                let result = if method == "Replace" {
                    ...
                    replacement_arguments.push(arguments.get(1).cloned().unwrap_or(Value::Null));
                ...
                )
            {
                let result = match method {
                    "MapColors" => apply_icon_map_colors(*datum, &arguments, &mut state.heap)
                        .map(|()| Value::Null),
                    "Blend" => {
                        apply_icon_blend(*datum, &arguments, &mut state.heap).map(|()| Value::Null)
                    }
